...
架构：上游协议由第一个真实请求确认，模型聚合为可路由目录，数据面使用唯一全局网关
...
网关不会为了探测协议或能力去调用上游推理接口（那会在上游真实计费）：模型的
wire protocol 由第一个真实请求确认；能力字段只取上游 `/models` 声明与 OpenRouter
公开元数据，两处都没有声明的模型保持未发布，而不是发一条探测请求去试探。
上游编辑后原协议确认会失效，下次请求重新确认；预设 Provider 同时重新获取官方
文档模型列表。
